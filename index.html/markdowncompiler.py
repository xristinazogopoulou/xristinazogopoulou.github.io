...
def compile_italic_star(line):
    ...
    for c in line:
        if c=='*':
            starcheck=True
        if starcheck==True and firststar==True:
            linetwo+=linefour
            secondstar=True
            firststar=False
...
def compile_strikethrough(line):
    ...
    for c in line:
        if c=='~':
            starcheck=True
            x+=1
        if starcheck==True and x%2==0 and inside==True:
            linetwo+=linefour
            secondstar=True
            inside=False
        elif starcheck==True and x%2==0 and inside==False:
            linetwo+=linethree
            inside=True
            secondstar=False
        if c!='~':
...
def compile_images(line):
    ...
    for p in line:
        if p==')':
            ...
            startpar=True
        elif endpar==False and startpar==True:
            sitelink+=p
    for b in line:
        if b ==']':
            endbracket=True 
